...
void QPaintBox::mousePressEvent(QMouseEvent *event){
    emit(paintBoxMouseDown(event->buttons(),event->pos().x(), event->pos().y()));
}

void QPaintBox::mouseReleaseEvent(QMouseEvent *event){
    emit(paintBoxMouseUp(event->buttons(),event->pos().x(), event->pos().y()));
}

void QPaintBox::mouseMoveEvent(QMouseEvent *event){
    emit(paintBoxMouseMove(event->buttons(),event->pos().x(), event->pos().y()) );
}
